...
inline float * get_vector(DataSet* DS, int idx) {
    return (float*) *(DS->data+idx);
}

inline void set_val(DataSet* DS, int data_i, int elem_i,
        float val) {
...
inline float distance(DataSet* P, int p1, int p2 ) {
    ...
    if(P->type == 2) // String data
    {
        if ( g_options.distance_type == 10 ) {
            ret = dice_distance(P->strings->at(p1), P->strings->at(p2));
        }
        else {
...
DataSet* read_lshkit(const char* fname) {
    ...
    // " In the beginning of the file are three 32-bit integers: ELEM-SIZE, SIZE, DIM. ELEM-SIZE is the size of the element, and currently the only valid value is 4, which is the size of float. SIZE is the number of vectors in the file and DIM is the dimension. The rest of the file is SIZE vectors stored consecutively, and the total size is SIZE * DIM * 4 bytes."
...
DataSet* read_ascii_dataset(const char* fname) {
    ...
    for(int i_vector=0; i_vector < N ; i_vector++) {
        ...
        *datap = (float*)malloc(DS->dimensionality * sizeof(float));
        ...
        char * ok = fgets(line,max_chars,fp);
        if(ok == NULL) {terminal_error("premature end of file");}
        /*printf("|%s |\n",line);*/
        pbuf = line;
